...
def slugify_uniquely(value, model, slugfield='slug'):
    """ Generate a unique slug for a given model.
        ...
        The slug is based on value (e.g. a title). """
    suffix = 0
...
def make_custom_datefield(f):
    ...
    if isinstance(f, DateTimeField):
        formfield.widget.format = '%d.%m.%Y %H:%M'
        formfield.widget.attrs.update({'class':'datePicker', 'readonly':'true', 'style': 'cursor: default !important;'})
    return formfield
